...
CREATE TABLE lx.platform_message_logs_local on cluster default_cluster (
    `id` UInt64,
    `news_log_id` String,
    `push_data` String,
    `push_platform` String,
    `platform_corp_id` String,
    `sent_user_count` Int32,
    `received_user_count` Int32,
    `success_message_id` String,
    `recall_status` Int8 DEFAULT 0,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/platform_message_logs/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, id)
SETTINGS index_granularity = 8192;

CREATE TABLE lx.platform_message_logs on cluster default_cluster (
    `id` UInt64,
    `news_log_id` String,
    `push_data` String,
    `push_platform` String,
...
ENGINE = Distributed('default_cluster', 'lx', 'platform_message_logs_local', cityHash64(toString(tuple(id))));
