...
-- kb_templates
CREATE TABLE IF NOT EXISTS lx.kb_templates_local on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `space_id` String,
    `name` String,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/kb_templates/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, incr_id)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS lx.kb_templates on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `space_id` String,
    `name` String,
...
ENGINE = Distributed('default_cluster', 'lx', 'kb_templates_local', cityHash64(toString(tuple(incr_id))));


-- kb_template_usages
CREATE TABLE IF NOT EXISTS lx.kb_template_usages_local on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `space_id` String,
    `template_id` String,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/kb_template_usages/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, incr_id)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS lx.kb_template_usages on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `space_id` String,
    `template_id` String,
...
ENGINE = Distributed('default_cluster', 'lx', 'kb_template_usages_local', cityHash64(toString(tuple(incr_id))));
