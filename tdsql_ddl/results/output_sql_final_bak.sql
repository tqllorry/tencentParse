...
CREATE TABLE IF NOT EXISTS lx.kb_permission_applications_local on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `target_type` String,
    `target_id` String,
    ...
    `created_by` String,
    `approved_by` String,
    `approved_at` Nullable(UInt64),
    `expired_at` Nullable(UInt64),
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/kb_permission_applications/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, incr_id)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS lx.kb_permission_applications on cluster default_cluster (
    `incr_id` UInt64,
    `id` String,
    `target_type` String,
    `target_id` String,
...
ENGINE = Distributed('default_cluster', 'lx', 'kb_permission_applications_local', cityHash64(toString(tuple(incr_id))));


-- ud_field_values
CREATE TABLE IF NOT EXISTS lx.ud_field_values_local on cluster default_cluster (
    `incr_id` UInt64,
    `field_id` String,
    `origin_field_id` String DEFAULT '',
    `entity_type` String,
    `entity_id` String,
    `value` String,
    `created_by` String,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/ud_field_values/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, incr_id)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS lx.ud_field_values on cluster default_cluster (
    `incr_id` UInt64,
    `field_id` String,
    `origin_field_id` String DEFAULT '',
    `entity_type` String,
...
ENGINE = Distributed('default_cluster', 'lx', 'ud_field_values_local', cityHash64(toString(tuple(incr_id))));


-- kb_permission_approvals
CREATE TABLE IF NOT EXISTS lx.kb_permission_approvals_local on cluster default_cluster (
    `incr_id` UInt64,
    `apply_id` String,
    `approver` String,
    `approved_at` Nullable(UInt64),
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/kb_permission_approvals/{shard}', '{replica}', _version)
ORDER BY (SrcCDBID, SrcDatabaseName, incr_id)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS lx.kb_permission_approvals on cluster default_cluster (
    `incr_id` UInt64,
    `apply_id` String,
    `approver` String,
    `approved_at` Nullable(UInt64),
...
ENGINE = Distributed('default_cluster', 'lx', 'kb_permission_approvals_local', cityHash64(toString(tuple(incr_id))));
